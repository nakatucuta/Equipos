...
<div class="form-group">
    ...
    <ul id="itemList" class="list-group">
        @foreach ($item as $developer)
            <li class="list-group-item available-item" data-id="{{ $developer->id }}">
                {{ $developer->tipo_item }} [<strong>{{ $developer->service_tag }}</strong>]
                <br><span style="font-size: .7rem">{{ $developer->user_name }}</span>
            </li>
        @endforeach
    </ul>
</div>
